...
class Sample : public ShaderExample {
	...
	{
		Vector3 pos = Vector3{ static_cast<float>((std::rand() % 101 +10) * 0.01),0,static_cast<float>((std::rand() % 201 - 100) * 0.008)};
		pos = pos * PrepareRotationMatrix(rand() % 360, Vector3{ 0,0,1 }, 0);
		pos = Normalize(pos) * distance;
		return pos;
	}
